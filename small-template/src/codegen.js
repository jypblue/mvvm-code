...
var ppcg = CodeGen.prototype

ppcg.walkRoot = function (astRoot) {
  this.walk(astRoot, '  ', '0')
}

ppcg.walk = function (node, indent, parentIndex) {
...
ppcg.genStat = function (node, indent, parentIndex) {
  var self = this
...
ppcg.genIfStat = function (node, indent, parentIndex) {
...
ppcg.genElseIf = function (node, indent, parentIndex) {
...
ppcg.genEachStat = function (node, indent, parentIndex) {
...
ppcg.genNode = function (node, indent, parentIndex) {
  ...
  this.lines.push(
    ...
    '", ' + ppcg.getAttrs(node) + ', ' + nodeName + '.children);'
  )
  this.lines.push(
    indent +
...
ppcg.genString = function (node, indent, parentIndex) {
  var line = indent + 'node' + parentIndex + '.children.push(' + getInterpolation(node) + ')'
  line = line.replace('\n', '\\n')
  this.lines.push(line)
}

ppcg.getAttrs = function (node) {
  var str = '{'
